...
2. **UI Deployment**: `k8s/ui-deployment.yaml`에서 `VITE_API_URL`/`VITE_API_HOST`를 빈 값으로 두고 Nginx 프록시를 함께 사용함
...
2. **브라우저 실행 환경**: UI는 사용자의 브라우저에서 실행되므로 클러스터 내부 주소(`http://api-service:8000`)에 접근 불가능
...
```yaml
env:
    # 런타임 env는 빈 값으로 두고 Nginx/sub_filter가 /api 경로를 보정
    - name: VITE_API_URL
      value: ''
    - name: VITE_API_HOST
      value: ''
...
```typescript
// 실제 코드는 VITE_API_HOST를 우선 사용하고, 비어 있으면 localhost:3000으로 fallback
const LOCAL_URL = 'http://localhost:3000'
export const domain = import.meta.env.VITE_API_HOST || LOCAL_URL
```
...
3. **포트**: 서비스에서 정의된 포트(8000)를 사용하여 통신
...
graph TB
    subgraph "Kubernetes Cluster (skald namespace)"
        UI[UI Deployment] -->|http://api-service:8000| API[API Service]
        API --> API_POD[API Pod]
        ConfigMap[ConfigMap<br/>skald-config] --> UI
    end

    subgraph "ConfigMap Data"
        INTERNAL_URL[INTERNAL_API_URL: http://api-service:8000]
...
1. **빌드 시점**: 프론트엔드 코드는 `import.meta.env.VITE_API_HOST`를 기준으로 번들됨
2. **배포 시점**: UI Deployment는 `VITE_API_URL`/`VITE_API_HOST`를 빈 값으로 두고 Nginx ConfigMap을 함께 배포
3. **런타임**: Nginx `sub_filter`와 `/api` 프록시가 브라우저 요청을 상대 경로(`/api`) 기반으로 보정함

### 환경별 설정

| 환경        | UI API 호출값           | 내부 프록시 대상          | 용도      |
| ----------- | ----------------------- | ------------------------- | --------- |
| Development | `http://localhost:8080` | 없음                      | 로컬 개발 |
| Staging     | `/api`                  | `http://api-service:8000` | 스테이징  |
| Production  | `/api`                  | `http://api-service:8000` | 운영      |
...
1. `k8s/ui-deployment.yaml` 수정 사항 확인 (이미 수정됨):
    - `VITE_API_URL`/`VITE_API_HOST`를 빈 값으로 유지
    - Nginx 설정 볼륨 마운트 추가
2. UI Deployment 재배포:
    ```bash
...
```bash
kubectl exec -it deployment/ui -n skald -- env | grep VITE_API_
# 예상 출력: VITE_API_URL= 와 VITE_API_HOST=
```
...
이제 UI는 상대 경로(`/api`)로 API를 호출하고, Nginx가 이를 클러스터 내부의 `api-service:8000`으로 프록시합니다. 이는 Kubernetes 환경에서 권장되는 표준 패턴이며, 확장성과 유지보수성이 뛰어납니다.
